...
class ClientEntity:
    ...
    def send_request_for_text(self, filename, uuid_client, request_type):
    ...
    def send_request_for_items(self, filename, uuid_client, request_type):
    ...
    def generate_file_chunks(self, filename):
        self.processed_chunks = 0
...
def read_recognizer_results(filename: str, uuid_client: str):
...
def read_anonymized_items(filename, uuid_client):
...
def check_required_files(filename, request_type):
    if request_type == 'anonymize':
...
def check_duplicates(entity_type, configuration_file):
